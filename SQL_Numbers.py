...
# Load the dataset
#df = pd.read_csv("arhivaLoto2.csv")

#Create a SQLite database and write the data to a table
#connection = sqlite3.connect("arhivaLoto.db")
#df.to_sql(name="arhivaLoto", con=connection, if_exists='replace', index=False)
...
# Load the database
db = SQLDatabase.from_uri("sqlite:///arhivaLoto.db")
...
write_report = Task(
    description="""Write an executive summary of the report from the analysis. 
                   ...
                   Print the Sql query in output log file""",
...
crew = Crew(
    ...
    verbose= True,
    memory=False,
    output_log_file="Sql_numbers.log",
)

inputs = {
    "query": """Count how many times each number between 1 and 49 exists in each column and sum the 6 columns for each number
                Print the top 10 numbers by max count in entire dataset.               
            """
}
